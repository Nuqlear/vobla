...
class Signup extends Component {
  ...
  handlePasswordConfirmChange = e => this.authStore.setPasswordConfirm(e.target.value);
  ...
  render() {
    const {
      authenticated, inProgress, errors, values, message
    } = this.authStore;
      ...
      <Auth message={ message } isProgress={ inProgress } question={ question }>
        {
          ...
          !this.authStore.inProgress &&
          <Redirect to='/' />
        }
        <form acceptCharset='UTF-8' role='form' className='form-auth' onSubmit={ this.handleSubmit }>
          <fieldset className={ inProgress ? 'op9' : ''}>
            ...
            <input
              ...
              disabled={inProgress}
            />
            <input type='submit' id='singup' value='Signup' disabled={
              inProgress || message ||
              !(
                values.invite_code &&
                values.email &&
