...
class UserFactory(BaseAsyncFactory):
    user_tier_id = models.UserTierEnum.default()
...
class UserTierFactory(BaseAsyncFactory):
    id = models.UserTierEnum.default()
    ...
    @staticmethod
    async def create_default(conn) -> models.UserTier:
        res = await UserTierFactory(conn=conn, id=models.UserTierEnum.default())
        return res
