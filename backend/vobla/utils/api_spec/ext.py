# api_spec tornado plugin from official repo with minor additions
# which fix issues with webargs lib
# -*- coding: utf-8 -*-
...
def tornadopath2swagger(urlspec, method):
    ...
    """
    named_groups = urlspec.regex.groupindex.keys()
    if len(named_groups) == urlspec.regex.groups:
        args = named_groups
    else:
        ...
        left_bound = args.index(named_groups[0])
        if left_bound:
            args = args[left_bound:left_bound+urlspec.regex.groups]
        else:
            right_bound = args.index(named_groups[-1])
            if right_bound:
                args = args[right_bound-urlspec.regex.groups:right_bound]
